...
| Caption swapping (pure) | `src/caption-swap.ts` | Replaces a caption and invalidates the fields indexed against the old words (`wordTimings`, `emphasisWords`). Shared by localization and hook variants — both are "same reel, different words" and break identically. |
| Hook variants (pure) | `src/variants.ts` | Builds one recipe per alternative opening and reports which segments differ, so only those get re-rendered. |
| Localization (pure) | `src/localize.ts` | Swaps a recipe's captions for a translation and invalidates what the swap breaks: per-sentence word timings, out-of-range emphasis indices, and the reading-speed assumption that a different-length language quietly violates. |
...
| Brand kits (pure) | `src/brand-kit.ts` | Merges a standing set of marks (overlays, caption defaults, fonts) into a recipe. Additive, idempotent, and recipe-wins-by-default: the recipe is the specific decision, the kit is the standing one. Orthogonal to presets — a preset is one reference's look, a kit is what survives changing reference. |
