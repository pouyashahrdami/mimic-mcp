...
| `render_reel` | Renders the Remotion project to an mp4. Pass `quality: "draft"` for a fast half-resolution preview while iterating, `"final"` for the deliverable. Final renders are **mixed to −14 LUFS** (two-pass `loudnorm`, video stream copied) — the loudness every platform re-gains to, so the reel doesn't land quiet. |
