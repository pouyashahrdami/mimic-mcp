...
| `track_subject` | **Follows** a subject that moves, instead of aiming at where it was. `suggest_framing` gives one fixed point per span — right for a locked-off shot, and it loses anyone who walks, since cropping 16:9 to 9:16 throws away two thirds of the width. This measures per window and returns keyframes for the segment's `backgroundTrack`. The series is smoothed and **speed-limited** so a measurement glitch can't become a whip pan, and a subject that barely moved comes back as a single position rather than a track that would only jitter. |
...
| `hook_variants` | Writes one recipe per alternative **opening**, changing nothing else — the first 1.5s is what creators actually A/B. Rebuilding a whole reel per hook both wastes a render on byte-identical frames and lets the variants differ in more than the thing under test, so each variant reports **which segments changed**: pass those to `render_reel`'s `segments` and a variant costs one segment instead of a reel. Each is re-scored for reading speed, because a stronger hook that's too long to read in the time available is not a better hook. |
...
| `localize_reel` | Puts the reel out in **another language**: swaps the captions for a translation you supply, and writes a renderable recipe plus `.srt`/`.vtt` for that language. It owns what goes silently wrong if you just swap the strings — `wordTimings` are offsets into a *specific sentence*, so a translated line would karaoke the **wrong words** (dropped unless you supply new ones); stale `emphasisWords` are cleared; and because languages aren't the same length, the translated reel is **re-scored for reading speed**, since a line that read fine in English can flash past in German at the same segment timing. |
